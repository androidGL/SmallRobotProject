...
import com.pcare.common.util.ScreenUtil;

/**
 * @Author: gl
 * @CreateDate: 2020/3/23
 * @Description: 滚轮滑动的选择器
 */
public class ScrollPickerView extends RecyclerView {
    ...
    public void onDraw(Canvas c) {
        ...
        if (!mFirstAmend) {
            mFirstAmend = true;
            ((LinearLayoutManager) getLayoutManager()).scrollToPositionWithOffset(getSelectedPosition(), 0);
        }
    }
    ...
    private int getSelectedPosition() {
        IPickerViewOperation operation = (IPickerViewOperation) getAdapter();
        if (operation != null) {
            return operation.getSelectedPosition();
        }
        return 1;
    }
